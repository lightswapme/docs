...
description: "Map LightSwap swap statuses, terminal states, and structured API errors to handling in your integration."
---

`GET /swap/{id}` returns the latest status for a swap created with your API key. Poll every 30 seconds while the status is not terminal.
...
Terminal statuses: `completed`, `refunded`, `failed`, `expired`.

### Underpaid swaps

`underpaid` is not terminal. The deposit arrived below the quoted amount and the swap is waiting for the route to settle the outcome. Keep polling: the swap can still move to `processing` and `completed`, or end as `refunded` or `failed`. Do not ask the user to send more funds unless LightSwap support tells you to.
...
| `unsupported_memo` | A memo was supplied for a network that does not support memos, or no route that can carry a memo is available for the pair. | Remove the memo and retry, or use a self-custody destination or another pair. |
...
| `quote_already_used` | The quote already created a swap. | Repeating `POST /swap` with the same `quoteId` returns the existing swap. If you still see this error, create a new quote. |
...
| `internal_error` | The request failed on the server. | Retry later. If it continues, contact support with your `externalId` and swap ID. |
...
For what to tell end users when a swap goes wrong, see [Refunds](/integrations/direct-api/refunds).
