...
description: "Authenticate LightSwap API requests with backend-only API keys and learn where those credentials must not be exposed."
---
...
You can request a separate test API key with the `lsk_test_` prefix for integration testing.

Test keys run against the live API with real liquidity, so every test swap is a real transaction. Use small amounts.

<Note>
  There is no sandbox environment. Request a test key from [LightSwap support](https://lightswap.me/support), and keep it separate from your production key.
</Note>
