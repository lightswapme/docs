...
The widget can run without a key. If you activated a business account on LightSwap, add your public widget key so swaps can be attributed to your integration.
...
<Steps>
  ...
  <Step title="Add a public widget key if needed">
    Add `key=YOUR_PUBLIC_WIDGET_KEY` from **Account → Business** when you want attribution and revenue sharing.
  </Step>
</Steps>
...
Copy your public widget key from **Account → Business** on LightSwap and add it as the `key` query parameter.
